...
bool CreateCSVFile (const char *filename_s, const char *col_sep_s, const char *row_sep_s, const json_t *headers_p, const json_t *data_p);
					...
					for (i = 0; i < num_columns; ++ i)
						{
							...
							const json_t *header_name_p = json_object_get (header_p, FD_TABLE_FIELD_NAME);

							if (header_name_p)
								{
									if (json_is_string (header_name_p))
										{
											const char *header_s = json_string_value (header_name_p);
